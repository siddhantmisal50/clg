/*
PS= A book consists of chapters, chapters consist of sections and sections consist of subsections. Construct a
...
void display(n *head)
{
	...
	for (int i = 0; i < head->count; i++)
	{
		...
		for (int j = 0; j < head->ptrChld[i]->count; j++)
		{
			...
			{
				cout << "\t---> Sub-Section " << (i + 1) << " : " << head->ptrChld[i]->ptrChld[j]->ptrChld[k]->name << endl;
			}
		}
	}
}
